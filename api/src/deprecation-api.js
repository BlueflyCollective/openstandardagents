...
/**
 * OSSA Deprecation Management API v0.1.3
 * 
...
function initializeDatabase() {
  ...
  if (!existsSync(SCRIPTS_DB)) {
    writeFileSync(SCRIPTS_DB, JSON.stringify({ scripts: {}, metadata: { version: '0.1.3', created: new Date().toISOString() } }, null, 2));
  }
...
app.post('/api/v1/migration/plan/:project', (req, res) => {
  try {
    ...
    const plan = {
      project,
      version: req.body.targetVersion || '0.1.3',
      created: new Date().toISOString(),
      steps,
...
app.listen(PORT, () => {
  console.log(`🚀 OSSA Deprecation Management API v0.1.3 running on port ${PORT}`);
  console.log(`📖 OpenAPI documentation: http://localhost:${PORT}/api/v1/deprecation-management-api-v0.1.3.yaml`);
