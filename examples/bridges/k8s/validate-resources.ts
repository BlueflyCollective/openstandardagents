...
function printResults(errors: ValidationError[]): number {
  ...
  if (errorList.length > 0) {
    ...
    for (const err of errorList) {
      ...
      console.log(
        `     Maximum allowed: ${err.max}${err.field.includes('cpu') ? 'm' : 'Mi'}`
      );
      console.log('');
    }
  }
  ...
  if (warningList.length > 0) {
    ...
    for (const warn of warningList) {
      ...
      if (warn.actual !== 'undefined') {
        console.log(
          `     Recommended max: ${warn.max}${warn.field.includes('cpu') ? 'm' : 'Mi'}`
        );
      }
      console.log('');
    }
...
export { RESOURCE_RULES, validateDirectory, validateManifest };
