...
describe('ServiceRegistry', () => {
  ...
  describe('Service Registration', () => {
    ...
    it('should validate required fields during registration', async () => {
      ...
      await expect(serviceRegistry.register(invalidService)).rejects.toThrow(
    ...
    it('should validate endpoint URL format', async () => {
      ...
      await expect(serviceRegistry.register(serviceData)).rejects.toThrow(
        'Service endpoint must be a valid URL'
      );
    });
  ...
  describe('Service Discovery', () => {
    beforeEach(async () => {
      ...
      await serviceRegistry.register(
        createTestService('ai-service', ['ai', 'nlp'])
      );
      await serviceRegistry.register(
        createTestService('data-service', ['data', 'etl'])
      );
      await serviceRegistry.register(
        createTestService('text-processor', ['ai', 'text'])
      );
    });
  ...
  describe('Service Health Management', () => {
    ...
    it('should emit health change events', (done) => {
      ...
      serviceRegistry.updateService('health-test-service', {
        health: testService.health,
      });
    });
  });
  ...
  describe('Service Updates', () => {
    ...
    it('should update service successfully', async () => {
      ...
      const updatedService = await serviceRegistry.updateService(
        'update-test-service',
        updates
      );
    ...
    it('should return null for non-existent service update', async () => {
      ...
      const result = await serviceRegistry.updateService(
    ...
    it('should emit update event', (done) => {
      ...
      serviceRegistry.updateService('update-test-service', {
        version: '2.0.0',
      });
    });
  });
  ...
  describe('Service Unregistration', () => {
    beforeEach(async () => {
      await serviceRegistry.register(
        createTestService('unregister-test-service')
      );
    });
    ...
    it('should unregister service successfully', async () => {
      ...
      const services = await serviceRegistry.discover({
        namePattern: 'unregister-test-service',
      });
  ...
  describe('Registry Statistics', () => {
    beforeEach(async () => {
      ...
      const service1 = await serviceRegistry.register(
        createTestService('healthy-service')
      );
      service1.health.status = ServiceHealthStatus.HEALTHY;
      await serviceRegistry.updateService('healthy-service', {
        health: service1.health,
      });

      const service2 = await serviceRegistry.register(
        createTestService('unhealthy-service')
      );
      service2.health.status = ServiceHealthStatus.UNHEALTHY;
      await serviceRegistry.updateService('unhealthy-service', {
        health: service2.health,
      });

      const service3 = await serviceRegistry.register(
        createTestService('degraded-service')
      );
      service3.health.status = ServiceHealthStatus.DEGRADED;
      await serviceRegistry.updateService('degraded-service', {
        health: service3.health,
      });
    });
  ...
  describe('Service Cleanup', () => {
    beforeEach(async () => {
      ...
      const oldService = await serviceRegistry.register(
        createTestService('old-service')
      );
      const newService = await serviceRegistry.register(
        createTestService('new-service')
      );

      // Manually set old timestamps to simulate aged services
      const pastDate = new Date(
        Date.now() - (config.serviceTtlSeconds + 1) * 1000
      );
      oldService.updatedAt = pastDate;
      await serviceRegistry.updateService('old-service', {
        updatedAt: pastDate,
      });
    });
  ...
  describe('Error Handling', () => {
    it('should handle Redis connection errors gracefully', async () => {
      ...
      await expect(errorRegistry.health('test-service')).rejects.toThrow(
        'Redis error'
      );
  ...
  describe('Event Handling', () => {
    it('should handle all event types correctly', (done) => {
      ...
      serviceRegistry.on('service:unregistered', () =>
        events.push('unregistered')
      );

      // Register, update, and unregister a service
      serviceRegistry
        .register(createTestService('event-test-service'))
        .then(() =>
          serviceRegistry.updateService('event-test-service', {
            version: '2.0.0',
          })
        )
