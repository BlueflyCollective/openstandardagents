...
describe('RedisEventBus', () => {
  ...
  describe('Connection Management', () => {
    ...
    test('should handle connection errors gracefully', async () => {
      ...
      mockInstance.connect.mockRejectedValueOnce(
        new Error('Connection failed')
      );

      const errorEventBus = new RedisEventBus(mockConfig);
      await expect(errorEventBus.connect()).rejects.toThrow(
        'Connection failed'
      );
    });
  ...
  describe('Stream Management', () => {
    ...
    test('should handle stream creation errors', async () => {
      const Redis = require('ioredis');
      const mockInstance = new Redis();
      mockInstance.xadd.mockRejectedValueOnce(
  ...
  describe('Contract Management', () => {
    ...
    test('should register event contract', async () => {
      ...
      await expect(eventBus.registerContract(contract)).resolves.not.toThrow();
    });
  });
  ...
  describe('Event Ordering and Reliability', () => {
    ...
    test('should handle event ordering with priorities', async () => {
      const events = [
        {
          type: 'test.priority.1',
          priority: EventPriority.LOW,
          data: { order: 3 },
        },
        {
          type: 'test.priority.2',
          priority: EventPriority.HIGH,
          data: { order: 1 },
        },
        {
...
describe('RedisEventBus Edge Cases', () => {
  ...
  test('should handle empty event data', async () => {
    ...
    await expect(eventBus.publish('test.empty', null)).resolves.toBeDefined();
  });
  ...
  test('should handle concurrent publish and subscribe operations', async () => {
    ...
    const publishPromise = eventBus.publish('test.concurrent', {
      data: 'test',
    });

    await expect(
      Promise.all([subscribePromise, publishPromise])
    ).resolves.toBeDefined();
  });
});
