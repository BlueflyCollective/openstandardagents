...
async function generateClient(options: GenerateOptions): Promise<void> {
...
async function generateServer(options: GenerateOptions): Promise<void> {
  ...
  try {
    ...
    const serverGenerators: Record<string, string> = {
...
async function generateAdapter(options: GenerateOptions): Promise<void> {
  ...
  try {
    ...
    const generator = adapterTemplates[options.framework as keyof typeof adapterTemplates];
    if (!generator) {
...
async function generateDiscovery(options: GenerateOptions): Promise<void> {
...
async function generateLangChainAdapter(outputPath: string, options: GenerateOptions): Promise<void> {
  const adapterCode = `
...
async function generateCrewAIAdapter(outputPath: string, options: GenerateOptions): Promise<void> {
  const adapterCode = `
...
async function generateOpenAIAdapter(outputPath: string, options: GenerateOptions): Promise<void> {
  const adapterCode = `
...
async function generateAnthropicAdapter(outputPath: string, options: GenerateOptions): Promise<void> {
  const adapterCode = `
...
async function generateSDKPackageJson(outputPath: string, language: string): Promise<void> {
  const packageJson = {
  ...
  await fs.writeJson(
    path.join(outputPath, language, 'package.json'), 
    packageJson, 
    { spaces: 2 }
  );
}