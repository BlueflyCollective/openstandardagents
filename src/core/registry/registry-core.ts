...
interface CapabilityIndex {
  domain: string;
  agents: Set<string>;
  subCapabilities: Map<string, Set<string>>;
}
...
export class RegistryCore extends EventEmitter {
  ...
  private readonly agentsByType = new Map<string, Set<string>>();
  private readonly agentsByTenant = new Map<string, Set<string>>();
   ...
   */
  async registerAgent(manifest: ACDLManifest, tenant?: string, namespace?: string): Promise<RegistrationResponse> {
    ...
    try {
      ...
      if (!validation.valid) {
        return {
          registrationId: '',
          status: 'rejected',
          validationResults: (validation as any).errors?.map((error: any) => ({
            check: error.field,
            passed: false,
   ...
   */
  async discoverAgents(query: DiscoveryQuery, tenant?: string): Promise<DiscoveryResponse> {
    const startTime = Date.now();
    ...
    try {
      ...
      for (const agentId of candidateAgents) {
        ...
        if (!registration || registration.status !== 'active') continue;
   ...
   */
  async matchAgents(request: MatchRequest, tenant?: string): Promise<MatchResponse> {
    const startTime = Date.now();
   ...
   */
  async updateAgentHealth(agentId: string, healthData: {
    responseTime?: number;
    success: boolean;
    error?: string;
  }): Promise<void> {
  ...
  getMetrics(): any {
    const healthyAgents = Array.from(this.agents.values()).filter(
      agent => agent.status === 'active' && agent.health.score > 0.7
    ).length;

    const degradedAgents = Array.from(this.agents.values()).filter(
      agent => agent.status === 'active' && agent.health.score <= 0.7 && agent.health.score > 0.3
    ).length;

    const unhealthyAgents = Array.from(this.agents.values()).filter(
      agent => agent.status === 'inactive' || agent.health.score <= 0.3
    ).length;
  ...
  private extractEndpoints(manifest: ACDLManifest): AgentRegistration['endpoints'] {
  ...
  private updateIndices(registration: AgentRegistration): void {
  ...
  private removeFromIndices(registration: AgentRegistration): void {
    const { agentId, manifest, tenant } = registration;
  ...
  private calculateCapabilityScore(manifest: ACDLManifest, query: DiscoveryQuery): number {
    let score = 0;
    let maxScore = 0;
    ...
    if (query.operations && query.operations.length > 0) {
      ...
      const matchingOperations = query.operations.filter(op =>
        agentOperations.includes(op)
      );
  ...
  private async calculateTaskCompatibility(
    manifest: ACDLManifest,
    request: MatchRequest,
    health: AgentRegistration['health']
  ): Promise<{ score: number; reasons: string[]; warnings: string[] }> {
    ...
    if (request.requirements.capabilities) {
      const capabilityMatch = await this.validator.validateCapabilityMatch(
        request.requirements.capabilities.domains || [],
    ...
    if (request.requirements.performance && manifest.performance) {
      const perfScore = this.calculatePerformanceScore(
        ...
        request.requirements.performance
      );
      score += perfScore * 0.3;
      if (perfScore > 0.8) {
  ...
  private calculatePerformanceScore(agentPerf: any, reqPerf: any): number {
    let score = 1.0;
  ...
  private generateRecommendation(matches: any[], request: MatchRequest): any {
    if (matches.length === 0) {
      return {
        primaryAgent: '',
        alternativeAgents: [],
  ...
  private determineEnsembleRole(agentId: string, index: number): string {
