...
export class SecurityScanner {
  ...
  async scanCode(
    ...
    vulnerabilities.push(
      ...this.scanForInjectionVulnerabilities(code, language)
    );
    ...
    recommendations.push(
      ...this.generateSecurityRecommendations(vulnerabilities, language)
    );
  ...
  private scanForInjectionVulnerabilities(
    code: string,
    language: string
  ): SecurityVulnerability[] {
    const vulnerabilities: SecurityVulnerability[] = [];
      ...
      if (matches) {
        matches.forEach((match) => {
          ...
          vulnerabilities.push({
            ...
            description:
              'Code appears to construct SQL queries through string concatenation, which may lead to SQL injection vulnerabilities.',
            line,
            recommendation:
        ...
        if (matches) {
          matches.forEach((match) => {
            ...
            vulnerabilities.push({
              ...
              description:
                'Code directly outputs user input to HTML without proper sanitization.',
              line,
              recommendation:
                'Sanitize and validate all user input before outputting to HTML.',
            });
          });
        }
      });
      ...
      if (matches) {
        matches.forEach((match) => {
          ...
          vulnerabilities.push({
            ...
            description:
              'Code executes system commands with user-controlled input.',
            line,
            recommendation:
              'Avoid executing system commands with user input. Use allow-lists and input validation.',
          });
        });
      }
    });
      ...
      if (matches) {
        matches.forEach((match) => {
          ...
          vulnerabilities.push({
            ...
            line,
            recommendation:
              'Store cryptographic keys in secure configuration files or environment variables.',
          });
        });
      }
    });
      ...
      if (matches) {
        matches.forEach((match) => {
          ...
          vulnerabilities.push({
            ...
            description:
              'Code contains or enforces weak password requirements.',
            line,
            recommendation:
              'Implement strong password policies with minimum length, complexity requirements, and avoid common passwords.',
          });
        });
      }
    });
      ...
      if (matches) {
        matches.forEach((match) => {
          ...
          vulnerabilities.push({
            ...
            line,
            recommendation:
              'Implement proper authentication checks and avoid hardcoded access controls.',
          });
        });
      }
    });
  ...
  private scanForSensitiveDataExposure(code: string): SecurityVulnerability[] {
    ...
    debugPatterns.forEach((pattern, index) => {
      ...
      if (matches && matches.length > 5) {
        // Only flag if many debug statements
        ...
        vulnerabilities.push({
          ...
          description:
            'Code contains many debug statements that may expose sensitive information.',
          line,
          recommendation:
            'Remove debug statements from production code or implement proper logging levels.',
        });
      }
    });
  ...
  private scanForInsecureConfigurations(
    code: string,
    language: string
  ): SecurityVulnerability[] {
    const vulnerabilities: SecurityVulnerability[] = [];
      ...
      if (matches) {
        matches.forEach((match) => {
          ...
          vulnerabilities.push({
            ...
            description:
              'Code uses insecure HTTP configurations that may expose data in transit.',
            line,
            recommendation:
              'Use HTTPS and enable SSL/TLS verification for all network communications.',
          });
        });
      }
    });
        ...
        if (matches) {
          matches.forEach((match) => {
            ...
            vulnerabilities.push({
              ...
              description:
                'Wildcard CORS configuration allows any origin to access resources.',
              line,
              recommendation:
  ...
  private generateSecurityRecommendations(
    ...
    if (vulnTypes.includes('CWE-327')) {
      recommendations.add(
        'Upgrade to stronger cryptographic algorithms (SHA-256, SHA-3, bcrypt)'
      );
    }
    ...
    if (language === 'javascript' || language === 'typescript') {
      recommendations.add(
        'Use ESLint security plugins for automated vulnerability detection'
      );
      recommendations.add(
