...
program
    ...
    try {
        ...
        const result = await complianceEngine.validateOSSAConformance(agent, context, options.frameworks);
        // Output results
...
program
    ...
    try {
        ...
        if (fs.statSync(input).isDirectory()) {
            ...
            const files = fs.readdirSync(input).filter(f => f.endsWith('.json') || f.endsWith('.yml') || f.endsWith('.yaml'));
        ...
        const report = await complianceEngine.generateComplianceReport(agents, context, options.frameworks);
        // Output results
        if (options.output === 'json') {
...
function outputTableResults(agent, result) {
...
function outputDetailedReport(agent, result, context, frameworks) {
...
function outputBatchTable(report) {
...
function outputBatchReport(report, context, frameworks) {
    ...
    report.agentResults.forEach((result) => {
        ...
        console.log(`      Findings: ${result.result.findings.length} (${result.result.findings.filter((f) => f.severity === 'critical').length} critical)`);
        console.log();
    });
