...
export class SpecificationValidator {
     ...
     */
    validateOSSACompliance(manifest) {
        const errors = [];
        const warnings = [];
        ...
        if (manifest.spec?.capabilities?.domains) {
            const invalidDomains = manifest.spec.capabilities.domains.filter((domain) => !this.validDomains.includes(domain));
    ...
    validateProtocols(protocols) {
        ...
        protocols.supported.forEach((protocol, index) => {
     ...
     */
    determineConformanceLevel(manifest) {
        ...
        const requirements = this.conformanceLevels[declaredLevel];
        if (!requirements)
            return 'bronze';
     ...
     */
    getTypeSpecificCapabilities(agentType) {
     ...
     */
    async validateCapabilityMatch(requiredCapabilities, agentManifest) {
