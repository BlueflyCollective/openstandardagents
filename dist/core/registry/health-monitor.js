...
export class HealthMonitor extends EventEmitter {
     ...
     */
    async initializeAgent(agentId, manifest) {
        const now = new Date();
        const lifecycle = {
            agentId,
    ...
    async performHealthCheck(agentId) {
        ...
        const now = new Date();
        const healthMetrics = { ...currentMetrics, timestamp: now };
        try {
            // 1. Endpoint health checks
            const endpointResults = await Promise.allSettled(currentMetrics.endpoints.map(endpoint => this.checkEndpoint(endpoint)));
            ...
            healthMetrics.responseTime = this.calculateResponseTimePercentiles(healthMetrics.endpoints.map(ep => ep.responseTime));
            // 5. Update error rate
    ...
    async simulateEndpointCheck(url) {
    ...
    calculateHealthScore(endpoints) {
    ...
    calculateAvailability(agentId, currentStatus) {
        const lifecycle = this.agentLifecycles.get(agentId);
        if (!lifecycle || lifecycle.healthHistory.length < 2)
            return 1.0;
    ...
    async updatePerformanceTrend(agentId, currentScore) {
    ...
    async checkSLAViolations(agentId, metrics) {
        const lifecycle = this.agentLifecycles.get(agentId);
        if (!lifecycle)
            return;
        let violations = 0;
    ...
    async updateLifecycleState(agentId, metrics) {
        const lifecycle = this.agentLifecycles.get(agentId);
        if (!lifecycle)
            return;
