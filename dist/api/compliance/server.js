...
app.post('/api/v1/compliance/validate', async (req, res) => {
    try {
        ...
        const result = await complianceEngine.validateOSSAConformance(agent, context, frameworks);
        res.json({
            success: true,
            data: {
...
app.post('/api/v1/compliance/validate/batch', async (req, res) => {
    try {
        ...
        const report = await complianceEngine.generateComplianceReport(agents, context, frameworks);
        res.json({
            success: true,
            data: {
...
export function startComplianceServer() {
