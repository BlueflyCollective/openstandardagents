# OSSA v0.1.3 Documentation

**Open Standards for Scalable Agents - Complete Documentation**

## 📚 Documentation Structure

### **API Documentation** (`api/`)
- [`ossa-api-reference.md`](api/ossa-api-reference.md) - Complete API reference
- [`agent-deployment.md`](api/agent-deployment.md) - Agent deployment APIs

### **User Guides** (`guides/`)
- [`getting-started.md`](guides/getting-started.md) - Quick start guide
- [`best-practices.md`](guides/best-practices.md) - OSSA best practices
- [`integration.md`](guides/integration.md) - Integration guide
- [`migration.md`](guides/migration.md) - Migration from v0.1.2
- [`enterprise-features.md`](guides/enterprise-features.md) - Enterprise capabilities

### **Technical Reference** (`reference/`)
- [`agent-specification.md`](reference/agent-specification.md) - Agent spec v0.1.3
- [`agent-discovery.md`](reference/agent-discovery.md) - ACDL discovery layer
- [`orchestration-specification.md`](reference/orchestration-specification.md) - Multi-agent orchestration

### **Development** (`development/`)
- [`development-guide.md`](development/development-guide.md) - Development setup
- [`installation.md`](development/installation.md) - Installation instructions
- [`publishing.md`](development/publishing.md) - Publishing guidelines


## 🚀 Quick Navigation

**New to OSSA?** Start with [`guides/getting-started.md`](guides/getting-started.md)

**Building agents?** See [`reference/agent-specification.md`](reference/agent-specification.md)

**API integration?** Check [`api/ossa-api-reference.md`](api/ossa-api-reference.md)

**Development?** Follow [`development/development-guide.md`](development/development-guide.md)

---

## 📋 Documentation Standards

- All files use kebab-case naming
- Organized by purpose and audience
- Cross-referenced with relative links
- Maintained for OSSA v0.1.3 compliance