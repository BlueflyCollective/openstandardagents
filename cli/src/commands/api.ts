...
function registerSystemCommands(program: Command): void {
...
function registerDiscoveryCommands(program: Command): void {
...
function displayMetricsTable(metrics: any): void {
...
function displayAgentDetails(agent: Agent): void {
...
function displayAgentHealthStatus(status: any): void {
...
function displayAgentCapabilities(capabilities: any): void {
  console.log(chalk.bold('Agent Capabilities'));
  
  if (capabilities.capabilities) {
    if (capabilities.capabilities.primary) {
      console.log('\nPrimary Capabilities:');
      capabilities.capabilities.primary.forEach((cap: string) => {
        console.log(`  • ${chalk.green(cap)}`);
      });
    }
    ...
    if (capabilities.capabilities.secondary) {
      console.log('\nSecondary Capabilities:');
      capabilities.capabilities.secondary.forEach((cap: string) => {
        console.log(`  • ${chalk.yellow(cap)}`);
      });
    }
  }
  ...
  if (capabilities.protocols) {
    console.log('\nSupported Protocols:');
    capabilities.protocols.forEach((protocol: any) => {
  ...
  if (capabilities.endpoints) {
    ...
    Object.entries(capabilities.endpoints).forEach(([name, url]) => {
      console.log(`  ${name}: ${chalk.cyan(url as string)}`);
    });
  }
}

function displayDiscoveryResults(results: any): void {
...
function displayRecommendations(recommendations: any): void {
  ...
  if (recommendations.task_analysis) {
    ...
    if (recommendations.task_analysis.required_capabilities) {
      ...
      recommendations.task_analysis.required_capabilities.forEach((cap: string) => {
...
async function promptAgentRegistration(): Promise<AgentRegistration> {
  ...
  return {
    ...
    spec: {
      ...
      capabilities: {
        primary: answers.capabilities.split(',').map((c: string) => c.trim())
      },
      endpoints: {
...
async function promptAgentUpdate(currentAgent: Agent): Promise<any> {
  const answers = await inquirer.prompt([
    {
      type: 'input',
      name: 'version',
  ...
  if (answers.updateCapabilities) {
    ...
    update.spec = {
      ...
      capabilities: {
        ...
        primary: capAnswers.capabilities.split(',').map((c: string) => c.trim())
      }
    };
  }
